...
RSpec.describe "Orders", type: :request do
  ...
  describe "POST /orders" do
    before(:all) do
      ...
      @new_user = create(:user, email: "newguytest@test.com", username: "newdude")
    end

    context "with token" do
      before(:each) do
        token = JwtService.encode(@new_user)
        post "/orders", headers: { Authorization: "Bearer #{token}" }, params: { order: { number: 12930, total: 99.99, user_id: 2 } }
      end
    ...
    context "without token" do
      before(:each) do
        post "/orders", params: { order: { number: 12930, total: 99.99, user_id: 2 } }
      end
